...
function processRecasts(reactions) {
    ...
    for (const reaction of reactions) {
      ...
      const username = reaction.fname;
      addUser(fid, username);
      awardPoints(fid, 40); // Award 10 points for each like or recast
    }
  }
...
function calculateUserPoints(cast) {
  ...
  const recastPoints = 40 * cast.reactions.recasts.length;
  const totalPoints = 169 + likePoints + recastPoints;
  return totalPoints;
}
